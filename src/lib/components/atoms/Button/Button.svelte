<script>
  ...
  import { Icon, useEventsAction } from '$ui'
  ...
  <a 
    ...
    {#if icon}
      {#if icon && position == 'left'}
        <Icon name={icon} {size} viewBox={box} class="-ml-2" />
        <slot/>
      {:else if icon && position == 'right'}
        <slot/>
        <Icon name={icon} {size} viewBox={box} class="-mr-2" />
      {:else}
        <Icon name={icon} {size} viewBox={box} />
      {/if}
    {:else}
      <slot/>
    {/if}
