<script>
	import { Base, Link } from '$lib';
	let {
		href = '#',
		icon,
		title,
		description,
		children = null,
		...props
	} = $props();
	// import IconDentistOutline from '~icons/fluent/dentist-20-regular';
	// import IconDentistFilled from '~icons/fluent/dentist-20-filled';
</script>

{#snippet template()}
	<Link
		href="#"
		class="card rounded-box preset-glass align-center grid max-w-xs place-items-center items-center justify-center border p-6 text-center shadow"
	>
		<svg
			class="text-primary-400-500 size-14"
			xmlns="http://www.w3.org/2000/svg"
			width="32"
			height="32"
			viewBox="0 0 20 20"
			><path
				fill="currentColor"
				d="M8.94 3.929c-.936-.757-1.975-.918-2.906-.736A4.26 4.26 0 0 0 3.777 4.46c-.603.656-1.195 1.486-1.241 2.7c-.022.602.067 1.475.532 2.129l.002.002c.613.851.922 1.475 1.048 2.287c.04.26.062.543.067.867a6.7 6.7 0 0 0 .467 2.334c.322.795.873 1.608 1.77 1.939l.022.008l.023.006c.524.14 1.012.09 1.38-.255c.324-.303.459-.754.535-1.16c.07-.376.105-.812.14-1.241l.012-.153c.04-.491.085-.98.18-1.442c.267-1.273.648-1.497.825-1.55c.491-.127.779-.041.97.098c.278.202.49.615.637 1.224c.143.593.2 1.265.251 1.877c.072.866.255 1.654.603 2.158c.181.262.438.49.785.565c.35.075.69-.03.99-.224c1.104-.704 1.743-2.167 1.918-4.105c.08-.871.168-1.294.297-1.628c.1-.26.224-.47.418-.802a39 39 0 0 0 .364-.632c.244-.436.644-1.23.69-2.16c.048-.966-.29-2.033-1.431-2.961c-1.09-.886-2.106-1.207-3.013-1.158c-.899.05-1.617.458-2.128.896l-.013.01l-.012.013c-.5.49-.73.577-.879.57c-.168-.006-.442-.137-1.006-.705l-.019-.02zm3.995 1.669c.49 0 .987.148 1.382.498c.399.354.647.873.706 1.526a.5.5 0 1 1-.996.09c-.04-.445-.197-.712-.373-.868c-.18-.16-.428-.246-.719-.246a.5.5 0 0 1 0-1"
			/></svg
		>
		<h3 class="mb-3 text-balance text-2xl font-medium">Meet Our Doctor</h3>
		<p class="text-neutral-500-300 text-balance" resizable>
			Trusted local dentist in Beaumont, providing expert and compassionate
			care.
		</p>
	</Link>
{/snippet}

{#snippet card()}
	<Link
		{href}
		class="rounded-box border-neutral-200-800 bg-neutral-50-950 align-center grid max-w-xs place-items-center items-center justify-center border p-6 text-center shadow"
	>
		<svg
			class="text-primary-400-500 size-14"
			xmlns="http://www.w3.org/2000/svg"
			width="32"
			height="32"
			viewBox="0 0 20 20"
			><path
				fill="currentColor"
				d="M8.94 3.929c-.936-.757-1.975-.918-2.906-.736A4.26 4.26 0 0 0 3.777 4.46c-.603.656-1.195 1.486-1.241 2.7c-.022.602.067 1.475.532 2.129l.002.002c.613.851.922 1.475 1.048 2.287c.04.26.062.543.067.867a6.7 6.7 0 0 0 .467 2.334c.322.795.873 1.608 1.77 1.939l.022.008l.023.006c.524.14 1.012.09 1.38-.255c.324-.303.459-.754.535-1.16c.07-.376.105-.812.14-1.241l.012-.153c.04-.491.085-.98.18-1.442c.267-1.273.648-1.497.825-1.55c.491-.127.779-.041.97.098c.278.202.49.615.637 1.224c.143.593.2 1.265.251 1.877c.072.866.255 1.654.603 2.158c.181.262.438.49.785.565c.35.075.69-.03.99-.224c1.104-.704 1.743-2.167 1.918-4.105c.08-.871.168-1.294.297-1.628c.1-.26.224-.47.418-.802a39 39 0 0 0 .364-.632c.244-.436.644-1.23.69-2.16c.048-.966-.29-2.033-1.431-2.961c-1.09-.886-2.106-1.207-3.013-1.158c-.899.05-1.617.458-2.128.896l-.013.01l-.012.013c-.5.49-.73.577-.879.57c-.168-.006-.442-.137-1.006-.705l-.019-.02zm3.995 1.669c.49 0 .987.148 1.382.498c.399.354.647.873.706 1.526a.5.5 0 1 1-.996.09c-.04-.445-.197-.712-.373-.868c-.18-.16-.428-.246-.719-.246a.5.5 0 0 1 0-1"
			/></svg
		>
		<h3 class="mb-3 text-balance text-2xl font-medium">{title}</h3>
		<p class="text-neutral-500-300 text-balance">
			{description}
		</p>
	</Link>
{/snippet}
<Base {...props} class="grid {props.class}">
	{#if children}
		{@render children()}
	{:else if title}
		{@render card()}
	{:else}
		{@render template()}
	{/if}
</Base>
